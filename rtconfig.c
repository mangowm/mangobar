...
// Validate profiles in a cJSON array (a root `[...]` config or the internal
// sequence parser's temporary array).
...
// Parse the non-standard JSONC form `object, object, ...` without requiring
// an enclosing array. The resulting temporary array reuses the normal profile
// validation and cleanup path.
static int parse_set_object_sequence(MangoConfigSet *set, const char *json,
  ...
  for (;;) {
    ...
    if (index > 0) {
      if (*cursor != ',') {
        cJSON_Delete(arr);
        return set_err(err, errsz,
                       "config[%d]: expected ',' between objects", index);
      }
      cursor++;
      skip_json_ws(&cursor);
      if (*cursor == '\0')
        break; // JSONC-style trailing comma
    }

    const char *end = NULL;
    cJSON *item = cJSON_ParseWithOpts(cursor, &end, 0);
    if (!item) {
      cJSON_Delete(arr);
      return set_err(err, errsz, "config[%d]: invalid JSON object", index);
    }
    if (!cJSON_IsObject(item)) {
      cJSON_Delete(item);
      cJSON_Delete(arr);
      return set_err(err, errsz, "config[%d]: must be an object", index);
    }
    if (cJSON_GetObjectItemCaseSensitive(item, "css") != NULL ||
        cJSON_GetObjectItemCaseSensitive(item, "style") != NULL) {
      cJSON_Delete(item);
      cJSON_Delete(arr);
      return set_err(err, errsz,
                     "config[%d]: 'css'/'style' are not allowed in root-object "
                     "sequence mode",
                     index);
    }
  ...
  int ret = parse_profiles(set, arr, err, errsz);
  cJSON_Delete(arr);
  return ret;
}
...
int mango_config_set_parse(MangoConfigSet *set, const char *jsonc, char *err,
  ...
  if (cJSON_IsObject(root) && *end == ',') {
    cJSON_Delete(root);
    root = NULL;
    ret = parse_set_object_sequence(set, stripped, err, errsz);
  } else if (*end != '\0') {
    cJSON_Delete(root);
    free(stripped);
    ...
    ret = set_err(err, errsz,
                  "config root must be an object, an array of objects, or "
                  "comma-separated root objects");
  }
